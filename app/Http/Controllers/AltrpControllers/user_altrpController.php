...
class user_altrpController extends ApiController
{
    ...
    public function get_roles_options(ApiRequest $request)
    {
        $entity = Model::where('name', 'user_altrp')->first()->altrp_sql_editors->where('name', 'get_roles_options')->first();

        $res = selectForSQLEditor(
        "SELECT `id` as `value`, `display_name` as `label` FROM `hack_roles` 
    WHERE `id` != 1",  [], [
           'sql_name' => 'get_roles_options',
           'table_name' => 'users',
         ], $request );
